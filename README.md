...
* **TimeSHAP:** "모델이 **왜** 그런 예측을 했는가?"를 다각도로 분석합니다.
    * **Local Analysis (개별 예측 설명):**
        * **Feature-Level(변수 중요도):** "어떤 변수가 이 예측에 중요했나?" 거래량, 금리, 심리지수 중 어떤 **변수**가 이번 예측을 주도했는지 파악합니다.
        * **Event-Level(시점 중요도):** "어느 시점이 이 예측에 중요했나?" 과거 14~45일(Lookback Window) 중 결정적이었던 **특정 시점**을 포착합니다.
        * **Cell-Level(변수X시점 중요도):** "언제, 어떤 변수가 함께 예측에 중요했나?" 특정 시점의 특정 변수(예: *3일 전의 거래량 급증*)가 미친 영향을 정밀 분석합니다.
